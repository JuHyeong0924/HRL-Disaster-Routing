...
**최종 업데이트:** 2026-03-31
**작성 언어:** Korean (한국어)
**문서 설명:** 본 문서는 프로젝트의 모든 클래스, 함수 시그니처, 텐서 차원(Shape), 데이터 흐름을 코드를 보지 않고도 이해할 수 있도록 상세히 기술한 해설서입니다. 2026-03-31 기준 최신 RL 분리학습 구조와 최근 진행 상황까지 포함합니다.

---

## 0. 현재 구현 상태 및 최근 진행 상황 (2026-03-31)
...
추가로, joint readiness를 무시하고 강제로 joint를 실행해야 할 때는 `--force_joint`를 사용합니다.
...
### 0.3 2026-03-31 기준 반영된 주요 구조 변경
#### A. Worker 인터페이스 정상화
...
#### B. Soft-arrival, adaptive skip, adaptive stagnation
...
#### D. Manager sparse warm-start
Manager-only stage는 학습 초반 `20%` episode 동안 sparse anchor target에 대한 CE 중심 warm-start를 수행합니다.

*   `warmstart_active=True`
*   `RL weight = 0.0`
*   `Aux CE weight = 1.0`

이후 나머지 `80%` 구간에서 RL + aux CE 혼합으로 전환합니다.
...
또한 마지막 subgoal 이후 goal이 active target이 된 직후 `8 step` 동안 worker aux CE에 `1.5x` handoff-aware boost를 줍니다.

#### G. Readiness gate와 artifact
...
### 0.4 현재까지의 최신 smoke 결과
2026-03-31 기준, 현재 코드가 실제로 동작하는지에 대한 최소 smoke 결과는 다음과 같습니다.

#### A. Joint 강제 smoke (`--stage joint --force_joint --episodes 1`)
현재 [rl_phase1_joint/rl_debug_log.txt](/d:/연구실/연구/재난드론/Code/logs/rl_phase1_joint/rl_debug_log.txt)에 남는 1-episode smoke는 다음을 보여줍니다.

*   `Success Rate = 6.2% (1/16)`
*   `Plan Length mean = 1.1`
*   `Anchor Err/Near = 1.56 / 81.2%`
*   `LastSG/Handoff = 25.0% / 25.0%`
*   `PostLastSG success = 25.0%`
*   `Stagnation Fail = 75.0%`
*   `Goal/SG per100 = -14.51 / 1.63`

해석:

*   새 handoff 로그와 stage-aware PBRS는 실제로 잘 찍히고 있음
*   다만 joint 자체의 성능이 좋아졌다는 뜻은 아니며, 여전히 Worker last-mile 안정성은 약함

#### B. Phase 1 pipeline smoke (`--stage phase1 --episodes 1`)
현재 [rl_phase1_manager/rl_debug_log.txt](/d:/연구실/연구/재난드론/Code/logs/rl_phase1_manager/rl_debug_log.txt), [rl_phase1_worker/rl_debug_log.txt](/d:/연구실/연구/재난드론/Code/logs/rl_phase1_worker/rl_debug_log.txt), [stage_readiness.json](/d:/연구실/연구/재난드론/Code/logs/rl_phase1_joint/stage_readiness.json)에 남는 latest smoke는 다음을 보여줍니다.

*   Manager smoke:
    *   `plan_under_rate = 0.0`
    *   `anchor_near_rate = 56.25%`
    *   `first_subgoal_hops_mean = 3.19`
*   Worker smoke:
    *   `stagnation_fail_rate = 12.5%`
    *   `goal_after_last_subgoal_rate = 100%`
    *   `post_last_sg_success_rate = 6.25%`
*   Readiness:
    *   `smoke_ready = true`
    *   `launch_ready = false`
...
*   manager/worker stage 자체는 정상적으로 실행됨
*   readiness artifact도 의도대로 생성됨
*   다만 현재 gate를 막는 주된 이유는 `post_last_sg_success_rate`가 아직 매우 낮다는 점

즉, 현재 코드는 “안 돌아가는 상태”는 아니며, **이제 실제 병목이 goal handoff 쪽임을 로그로 직접 확인할 수 있는 상태**입니다.

### 0.5 현재 결론
현재 프로젝트는 “component가 완전히 죽은 상태”는 아닙니다. 더 정확히 말하면 다음과 같습니다.

*   **Manager**
    *   조기 EOS와 길이 1 collapse는 상당 부분 완화됨
    *   그러나 hard case에서 first-segment budget overshoot와 anchor 품질 저하가 여전히 남아 있음
*   **Worker**
    *   active subgoal 근처까지 가는 로컬 tracking은 가능
    *   그러나 마지막 subgoal 이후 goal까지 마무리하는 success는 아직 낮음
*   **Joint RL**
    *   이제 handoff/goal progress/segment budget을 직접 볼 수 있는 계측이 생김
    *   하지만 readiness를 통과하기 전까지는 구조적 문제를 해결하는 단계라기보다 noise를 증폭시키는 단계에 가까움

### 0.6 당장 다음에 확인해야 할 핵심 지표
Phase 1 재실행 시, corridor보다 아래 지표를 우선적으로 봅니다.
...
*   `first_segment_budget_err_mean`
*   `frontloaded_overshoot_rate`
*   `reached_last_subgoal_rate`
*   `goal_after_last_subgoal_rate`
*   `goal_dist_after_last_sg_mean`
*   `steps_after_last_sg_mean`
*   `post_last_sg_success_rate`
*   `post_last_sg_stagnation_rate`
*   `soft_arrival_rate`
*   `skip_rate`
...
*   `plan_under_rate`, `anchor_near_rate`, `first_subgoal_hops_mean`, `segment_budget_error_mean`
    *   Manager sparse planning 품질의 핵심
*   `goal_after_last_subgoal_rate`, `post_last_sg_success_rate`, `stagnation_fail_rate`
    *   Worker/Joint handoff 안정성의 핵심
...
*   **Workflow**:
    ...
    3.  **Phase 3: Visualization & Evaluation (`tests/visualize_result.py`)**:
        *   학습된 모델(SL, RL Phase 1, RL Phase 2)을 각각 별도로 로드하여 성능 지표 추출 및 맵 경로 시각화를 수행합니다.
        *   **`--eval_episodes`**: 병렬 시뮬레이션을 통해 통계학적 성능 표(Success Rate, Path Length Ratio 등)를 도출하는 대량 테스트 횟수 (기본값: 100)
        *   **`--viz_episodes`**: 평가 에피소드 중, 눈으로 확인 가능한 실제 맵 경로 시각화 이미지(.png)를 렌더링할 횟수 (기본값: 5)
        *   렌더링된 이미지는 테스트 모드에 따라 `tests/hierarchical/[맵이름]/SL`, `RL_phase_1`, `RL_phase_2` 폴더로 자동 분류되어 저장됩니다.

### 2.2 학습 루프 상세 (Training Loop Details)
#### **RL Loop (`train_rl.py`, 2026-03-31 기준)**

현재 [train_rl.py](/d:/연구실/연구/재난드론/Code/train_rl.py)는 `_run_stage()`와 `train_rl()` 두 층으로 구성됩니다.
...
2.  **Model 생성**
    ...
    *   `WorkerLSTM(node_dim=7, hidden_dim=args.hidden_dim)`
3.  **Checkpoint 로드**
...
*   near 상태이면서
    *   마지막 target이거나
    *   `hop(curr, next_target) + 1 <= hop(sg, next_target)`이면
    *   `soft-arrival` 허용
...
### D. Manager-only Stage의 Plan Score
Manager-only stage는 실제 worker rollout reward 대신, **plan 자체의 구조적 품질 점수**를 직접 최적화합니다.

현재 [manager_stage_trainer.py](/d:/연구실/연구/재난드론/Code/src/trainers/manager_stage_trainer.py)의 점수 구성은 다음과 같습니다.

*   `R_count`
    *   `-1.5 * under_count - 0.20 * over_count`
*   `R_anchor`
    *   `-0.30 * anchor_hop_error_mean + 0.75 * anchor_near_ratio`
*   `R_spacing`
    *   `-1.20 * spacing_error_mean`
*   `R_mono`
    *   `-0.80 * monotonic_violation_mean`
*   `R_budget`
    *   `-0.80 * segment_budget_error_mean`
*   `R_front`
    *   `-1.00 * first_segment_overshoot`
*   `R_first`
    *   첫 subgoal이 기대 첫 anchor보다 너무 먼 경우와 첫 segment budget error에 대한 penalty
*   `R_corr`
    *   corridor deficit이 클 때만 약하게 penalty
*   `R_empty`
    *   빈 계획이면 `-2.0`

즉, Manager-only stage는 이제 단순한 길이 보정보다 **segment budget과 anchor 배치 품질**을 더 직접적으로 학습합니다.

#### Sparse Warm-start
Manager stage 초반 `20%` episode는 sparse anchor sequence CE 중심 warm-start입니다.

*   `RL weight = 0.0`
*   `Aux weight = 1.0`

이후 나머지 구간에서 RL + auxiliary CE 혼합으로 전환합니다.
...
- **Manager (Graph Transformer Manager)**:
  ...
  - **Decoder (Pointer Network)**:
    - Standard Transformer Decoder (Attend to All Encoded Nodes).
    - Output: Suggests next node index (Pointer capability).
- **Worker (Local Planner)**
    - Model: `WorkerLSTM` (GATv2 + LSTM)
    - **Main Method**: `predict_next_hop` (Replaces `forward`)
    - **Features**:
        - **Detach Spatial**: Option to freeze GATv2 during RL to save VRAM.
        - **Dropout Consistency**: `F.dropout` applied in GAT loop for regularization.
        - **Robust Batching**: Handles missing `is_current` nodes gracefully (Zero Padding).
    - Input: Node Features (7 dim), Edge Index
    - Output: Next Hop Distribution (Softmax)
    - **Max Steps per Episode**: 300 (Increased from 100 for large map coverage)
    
- **Reward Function (POMO) [v3 - 2026-02-11]**
    - **Per-Step Shaping**: APSP 거리 감소량 / 최적거리 × 10.0 (최적경로면 총합 +10)
    - **Completion Bonus**: `max(0, 50 × (2.0 - ratio))` (ratio = 실제스텝/최적스텝)
    - **Subgoal Reward**: +3.0 per checkpoint 도달
    - **Checkpoint Quality** (방안 C): α=0.6 × 효율성 + β=0.4 × 균등분할 × 10
    - **Failure Penalty**: -5.0 (과도하지 않게)
    - **Loop Constraint**: Visit Count > 10 triggers immediate failure (Early Exit) with penalty equivalent to Max Steps.bability ($\mathcal{P}$)**:
    *   $P(s_{t+1} | s_t, a_t) = 1$ (Deterministic Sequence Generation).
    *   $H_{t+1} = H_t \cup \{a_t\}$.
    *   그래프 $G$는 고정됨.

4.  **Reward Function ($\mathcal{R}$)**:
    *   $r_t = r_{efficicncy} + r_{outcome}$
    *   **Efficiency**: $r_{efficiency} = -0.5$ (Checkpoint Step Cost). 불필요한 체크포인트 생성을 억제.
    *   **Outcome**:
        *   $r_T = +50.0$ if $c_T = Goal$.
...
*   **Architecture (v2)**:
    ...
    *   **Edge Feature**: `edge_dim=5` ([length, damage, is_closed, is_danger, speed] - Phase 1에서는 정상 상태 패딩).
    *   **Memory**: `LSTMCell` (Path History).
    *   **Scorer**: **4-head Multi-head Scorer** (독립 MLP 4개 → 평균 앙상블).
    *   **Input**: $x_{local} = [Pos(2), Is\_Robot, Is\_Target, Net\_Dist, Dir(2)]$. (Total 7 Dims)
    *   **Output**:
...
*   **Vectorized Worker Execution**:
    ...
    *   **Logic**:
        ...
        *   **[Fix 2026-03] Dynamic Edge Feature**: `exec_batch_plan` 내부에서 Worker의 `predict_next_hop` 호출 전, 동적인 물리 환경 변화를 반영하기 위해 매 스텝 `ea = pyg_data.edge_attr[:, [0, 1, 4, 6, 8]]`를 새로 추출하여 5D 텐서로 주입. (NameError 방지 및 부분 특성 적용)
...
**`tests/visualize_result.py`** (`evaluate_metrics()`)

A* 최적 경로 대비 모델 성능을 정량적으로 평가.

| 모델 계층 | 평가 지표 | 정의 | 목표 |
|---|---|---|---|
| Manager | **Sub-goal Hit Rate** | 예측 서브골이 A* 경로 위에 존재하는 비율 | 80% ↑ |
| Worker | **Next-Hop Accuracy** | Worker 선택이 A*와 일치하는 비율 (Top-1) | 90% ↑ |
| Overall | **Path Deviation** | A* vs 모델 경로 Hausdorff Distance | 낮을수록 ↓ |
| Overall | **Path Length Ratio** | 모델 경로 거리 / A* 최적 거리 | ≤ 1.2 |
| Overall | **Success Rate** | 목표 노드 도달 비율 | 95% ↑ |

*   **실행**: `python tests/visualize_result.py --episodes 50 --mode sl`
*   **출력**: 에피소드별 지표 + 전체 요약 + `evaluation_metrics.json` 저장.

#### **Dynamic EOS Token Handling in Visualization [New]**
*   **Problem**: Manager의 Pointer Network가 예측하는 EOS_TOKEN 인덱스는 맵의 노드 개수(`num_nodes`)에 따라 가변적임. (예: SiouxFalls는 24) 하드코딩된 `-2` 검사 시 `KeyError`가 발생할 수 있음.
*   **Solution**: `visualize_result.py`의 `hierarchical_rollout` 디코딩 로직을 개선하여 `t_val >= env.num_nodes`인 경우 시퀀스 생성을 조기 종료(break)하도록 수정함.
