...
**문서 버전:** 3.9
**최종 업데이트:** 2026-04-18 (v3.9: Manager Stage 학습 안정화 + 보상 해킹 방지 + Worker 시각화 버그 수정 + POMO 48 + 평가 POMO 추론)
**작성 언어:** Korean (한국어)
...
| 변경 파일 | 핵심 수정 |
|---|---|
| `disaster_env.py` | `max_hops` 캐싱, reset() 10채널, update_target_features() col 9, get_reward() hop dense reward, forced_start/target |
| `worker.py` | `node_dim=9` 기본값, docstring |
| `train_sl.py` | `WorkerLSTM(node_dim=9)`, worker_in 9채널 |
| `train_rl.py` | `WorkerLSTM(node_dim=9)`, 레거시 적응 일반화 |
| `pomo_trainer.py` | 피처 슬라이싱 자동 호환 (env_x[:, 5:] → 5채널) |
| `eval_core.py` | node_dim=9, ensure_env_layout 10채널, build_worker_input 10→9 |
| `evaluate.py` | node_dim=9 |
